...
class TestPolicyChunker:
    ...
    def test_split_returns_list(self):
        ...
        docs = [Document(page_content="Some policy text.", metadata={"source": "test.pdf", "page": 1})]
    ...
    def test_metadata_preserved(self):
        chunker = PolicyChunker()
        docs = [Document(page_content="Some policy text " * 100, metadata={"source": "policy.pdf", "page": 1})]
        chunks = chunker.split(docs)
    ...
    def test_cover_page_filtered(self):
        chunker = PolicyChunker()
        docs = [Document(page_content="COVER PAGE", metadata={"source": "test.pdf", "page": 0})]
        chunks = chunker.split(docs)
        assert len(chunks) == 0