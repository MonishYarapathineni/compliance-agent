...
def run_ragas_eval(dataset=None) -> dict:
    ...
    ragas_llm = LangchainLLMWrapper(
        ChatOpenAI(model="gpt-4o-mini", max_tokens=4096, temperature=0.0)
    )
    ...
    with mlflow.start_run(run_name="ragas_eval_v1"):
        ...
        mlflow.log_metric(
            "answer_relevancy", result.to_pandas()["answer_relevancy"].mean()
        )
...
def build_ragas_dataset(
    queries: list, answers: list, contexts: list, ground_truths: list
) -> object:
